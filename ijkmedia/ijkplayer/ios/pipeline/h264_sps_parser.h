...
enum H265_NAL_TYPE {
    NAL_H265_IDR_SLICE = 19,
    NAL_H265_IDR_SLICE_N_LP = 20,
};
...
static int ff_get_h265_nal_units_type(const uint8_t * const data) {
...
static bool ff_avpacket_is_idr(const AVPacket* pkt, bool is_h265) {
    ...
    if (pkt->data && pkt->size >= 5) {
        ...
        while (offset >= 0 && offset + 5 <= pkt->size) {
            ...
            state = is_h265 ? (ff_get_h265_nal_units_type(nal_start) == NAL_H265_IDR_SLICE) : (ff_get_nal_units_type(nal_start) == NAL_IDR_SLICE);
            if (state) {
                return true;
            }
...
static bool ff_avpacket_i_or_idr(const AVPacket* pkt, bool is_h265) {
    return ff_avpacket_is_idr(pkt, is_h265) || ff_avpacket_is_key(pkt);
}
