...
int videotoolbox_video_thread(void *arg)
{
    ...
    opaque->context->queue_frame_to_picture(opaque->context->opaque);
    
    for (;;) {

        if (is->abort_request || d->queue->abort_request) {
            opaque->context->wait_thread_finish(opaque->context->opaque);
            return -1;
        }
        @autoreleasepool {
...
IJKFF_Pipenode *ffpipenode_create_video_decoder_from_ios_videotoolbox(FFPlayer *ffp)
{
    ...
    switch (opaque->avctx->codec_id) {
        ...
        case AV_CODEC_ID_HEVC:
            if(version < 11.0) {
                ALOGI("os version is: %f, do not support hevc\n", version);
                goto fail;
            }
